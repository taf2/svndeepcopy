...
class TestCopy < Test::Unit::TestCase
  ...
  def test_copy_with_outside_repo
    ...
    @copier.expects(:capture_invoked).with("svn info #{repo_path}/applications/ruby/newsletters/branches/testit/vendor/plugins/mocha").times(1).returns(nil)
    @copier.expects(:capture_invoked).with("svn info #{repo_path}/applications/ruby/newsletters/branches/testit/vendor/plugins/rails_rcov").times(1).returns(nil)
    @copier.expects(:capture_invoked).with("svn info #{repo_path}/portal/content/trunk/test/fixtures/content").times(1).returns(load_fixture("info.fixture"))
    @copier.expects(:capture_invoked).with("svn info #{repo_path}/applications/ruby/newsletters/branches/testit/test/fixtures/content").times(1).returns(nil)
